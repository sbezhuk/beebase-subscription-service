package apple

import (
	"crypto/x509"
	"sync"
)

// Apple Root CA certificates in PEM format.
// Sourced from Apple Public Data Security PKI repository (https://www.apple.com/certificateauthority/).
const (
	// Apple Root CA - G3 (EC root, primary for App Store Server Notifications V2)
	appleRootCAG3PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	// Apple Root CA - G2 (RSA root)
	appleRootCAG2PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	// Apple Root CA
	appleRootCAPEM = `-----BEGIN CERTIFICATE-----

UKqK1drk/NAJBzewdXUh
-----END CERTIFICATE-----`
)

var (
	defaultAppleRootPool *x509.CertPool
	initRootOnce         sync.Once
)

// AppleRootCertPool returns a singleton x509.CertPool loaded with Apple's Root CAs.
func AppleRootCertPool() *x509.CertPool {
	initRootOnce.Do(func() {
		defaultAppleRootPool = x509.NewCertPool()
		defaultAppleRootPool.AppendCertsFromPEM([]byte(appleRootCAG3PEM))
		defaultAppleRootPool.AppendCertsFromPEM([]byte(appleRootCAG2PEM))
		defaultAppleRootPool.AppendCertsFromPEM([]byte(appleRootCAPEM))
	})
	return defaultAppleRootPool
}
